...
function assign_commentex( $id, $type, $page = 1, $flag = 0 )
{
		...
		$arr = array( );
		$ids = "";
		while ( ( $row = $GLOBALS['db']->fetchRow( $res ) ) ) // 修改 By  swtshop.cn
		{
				$ids .= $row['comment_id'];
...
function get_hot_cat_tree( $pid = 0, $rec = 4 )
{
		$arr = array( );
...
function get_hot_cat_tree2( $pid = 0, $rec = 3 )
{
		...
		foreach ( $res as $row )
		{
				$arr[$row['cat_id']]['id'] = $row['cat_id'];
				$arr[$row['cat_id']]['name'] = $row['cat_name'];
				$arr[$row['cat_id']]['cat_desc'] = $row['cat_desc'];
				$arr[$row['cat_id']]['keywords'] = $row['keywords'];
				$arr[$row['cat_id']]['url'] = build_uri( "category", array(
						"cid" => $row['cat_id']
				), $row['cat_name'] );
		}
...
function get_top()
{
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('ad') . " WHERE ad_id = 102";
...
function get_sale()
{
	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('ad') . " WHERE ad_id = 89";
 ...
 * 
 */
function get_teleft($name)
{

	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('ad') . " WHERE ad_name  = '$name'";
...
//小轮播图
function get_tesmall($ad_name)
{

	$sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('ad') . " WHERE ad_name  like '".$ad_name."%'";
...
//小轮播图-下图
function get_smabottom($ad_name)
{
...
//右上小图
function get_rightsamll($ad_name)
{
...
//右下小图
function get_rightbott($ad_name)
{
