...
else
{
    ...
    {
        /* 代码修改_START   By    www.swtshop.cn   */
		include_once('includes/lib_splitword_www_swtshop_com.php');
		$Recordkw = str_replace(array("\'"), array(''), trim($_REQUEST['keywords']));
		$cfg_soft_lang_www_swtshop_com = 'utf-8';
        $sp_www_swtshop_com = new SplitWord($cfg_soft_lang_www_swtshop_com, $cfg_soft_lang_www_swtshop_com);
        $sp_www_swtshop_com->SetSource($Recordkw, $cfg_soft_lang_www_swtshop_com, $cfg_soft_lang_www_swtshop_com);
        $sp_www_swtshop_com->SetResultType(1);
        $sp_www_swtshop_com->StartAnalysis(TRUE);
        $word_www_swtshop_com = $sp_www_swtshop_com->GetFinallyResult(' '); 
		//echo  $word_www_swtshop_com;
        $word_www_swtshop_com = preg_replace("/[ ]{1,}/", " ", trim($word_www_swtshop_com));
        $replacef_www_swtshop_com = explode(' ', $word_www_swtshop_com);


        $keywords = 'AND (';
        $goods_ids = array();
        foreach ($replacef_www_swtshop_com AS $key => $val)
        {
            if ($key > 0 && $key < count($replacef_www_swtshop_com) && count($replacef_www_swtshop_com) > 1)
            {
                $keywords .= " OR ";
            }
   ...
   /* fulltext_search_add_START_www.swtshop.cn */
    if($_CFG['fulltext_search'] == '0'){
   /* fulltext_search_add_END_www.swtshop.cn */
    /* 获得符合条件的商品总数 */
    ...
    $count = $db->getOne($sql);
	/* 代码添加_START   By  www.swtshop.cn  */	
    if ($page == 1 && $Recordkw && $count && !$category) Recordkeyword($Recordkw, $count, 'ecshop');  // 代码修改  By   www.swtshop.cn
	if($count==0)
	{
		if (preg_match('/^[a-zA-z]+$/i', $_REQUEST['keywords']))
		{
			$sql_www_swtshop_com = "select keyword from ". $ecs->table('keyword') ." where letter='". $_REQUEST['keywords'] ."' order by items desc limit 0,1";
			$keyword_www_swtshop_com = $db->getOne($sql_www_swtshop_com);
			if($keyword_www_swtshop_com && $keyword_www_swtshop_com!=$_REQUEST['keywords'])
			{
				header('Location: search.php?keywords='.$keyword_www_swtshop_com.'&keyword_zm='.$_REQUEST['keywords']);
				exit;
			}
		}
	}
	...
	{
		$smarty->assign('beizhuxinxi_www_swtshop_com', '关键词<font color=#cc0000>'.$_REQUEST['keyword_zm'].'</font>搜索结果为零，<br>但是我们为您匹配到了相关关键词<font color=#cc0000>'.$_REQUEST['keywords'].'</font>，下面是它的查询结果！');
	}
	/* 代码添加_END   By  www.swtshop.cn  */
...
//	$ts_string_array = array("*","/");
    ...
    if($_CFG['fulltext_search'] == '1'){
		...
		if(empty($idarray)){
			$idarray = array();
				foreach($replacef_www_swtshop_com as $key => $value){
    ...
    while ($row = $db->FetchRow($res))
    {
		...
		$arr[$row['goods_id']]['goods_name'] = $row['goods_name'];
		/* 代码添加_START   By    www.swtshop.cn */
		$arr[$row['goods_id']]['goods_name_www_swtshop_com'] =  $arr[$row['goods_id']]['goods_name'];
		foreach($replacef_www_swtshop_com as $key_www_swtshop_com =>$temp_qq)
		{
				//$replacet[$key_www_swtshop_com]=  '<strong style="color:#cc0000;">'. $temp_qq .'</strong>';;
				
				if(in_array($temp_qq,$ts_string_array)){
					$arr[$row['goods_id']]['goods_name_www_swtshop_com'] = preg_replace('/(?!<[^>]*)\\'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_swtshop_com']);
				}else{
					$arr[$row['goods_id']]['goods_name_www_swtshop_com'] = preg_replace('/(?!<[^>]*)'.$temp_qq.'(?![^<]*>)/i', '<strong style="color:#cc0000;">'. $temp_qq .'</strong>', $arr[$row['goods_id']]['goods_name_www_swtshop_com']);
				}
