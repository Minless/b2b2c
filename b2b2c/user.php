...
/*余额额支付密码_更改_START_www.swtshop.cn*/
'act_forget_pass', 're_pass', 'auction_list', 'forget_surplus_password', 'act_forget_surplus_password', 'update_surplus_password', 'act_update_surplus_password', 'verify_reset_surplus_email', 'get_verify_code','qrcode'
); // 代码修改
   // By
   // www.swtshop.cn
/* 余额额支付密码_更改_END_www.swtshop.cn */
...
/* 代码增加_Start By www.swtshop.cn  用户中心获取会员信息*/
...
function action_default ()
{
	...
	// 获取全局变量
    /* 代码增加 By  www.swtshop.cn Start */
    $_CFG = $GLOBALS['_CFG'];
    /* 代码增加 By  www.swtshop.cn End */
	$_LANG = $GLOBALS['_LANG'];
	$smarty = $GLOBALS['smarty'];
	$db = $GLOBALS['db'];
	$ecs = $GLOBALS['ecs'];
	$user_id = $_SESSION['user_id'];
	/* 代码注释 by www.swtshop.cn _star 路由前已调用会员信息，去掉重复调用*/
	$rank = get_rank_info();
	/* 	include_once (ROOT_PATH . 'includes/lib_clips.php');
	 if($rank = get_rank_info())
	 {
	...
	/* 代码增加--cb--推荐分成-- by www.swtshop.cn _star */
	$rn = $rank['rank_name'];
	$recomm = $db->getOne("SELECT is_recomm FROM " . $GLOBALS['ecs']->table('user_rank') . " WHERE rank_name= '$rn'");

    $smarty->assign('recomm', $recomm); // 获取当前用户是否是分成用户判断是否显示我的推荐
	/* 代码增加--cb--推荐分成-- by www.swtshop.cn _end */
...
function action_getverifycode ()
{
	...
	$content = '您的验证码为' . $verifycode . '【swtshop】';
	/* 发送注册手机短信验证 */
...
// 代码增加--swtshop--侧边栏登录 判断登录是否开启验证码
...
function action_signin ()
{
	...
	if($user->login($username, $password))
	{
		...
		recalculate_price(); // 重新计算购物车中的商品价格   并获取登录前购物车中的商品
...
function action_act_edit_password ()
{
	...
	/* 代码添加_swtshop_20150729_STAR */
	$confirm_password = isset($_POST['confirm_password']) ? trim($_POST['confirm_password']) : '';
	/* 代码添加_swtshop_20150729_END */
...
//	$order['shipping_status'] = $_LANG['ss'][$order['shipping_status']];
    /* 代码删除 By  www.swtshop.cn End */
	/* 增值税发票_添加_START_www.swtshop.cn */
	/* 增值税发票收票地址 */
	...
	foreach($goods_list as $goods_key => $goods_val)
	{
		...
		/* 代码增加_start By www.swtshop.cn _20150804 */
		...
		/* 代码增加_end By www.swtshop.cn _20150804 */
...
function action_act_account ()
{
	...
	{
		$surplus['defaultbank'] = isset($_POST['www_swtshop_com_bank']) ? trim($_POST['www_swtshop_com_bank']) : '';
	}
...
function action_act_edit_payment ()
{
	...
	{
		$defaultbank = $_POST['www_swtshop_com_bank'];
...
function action_qrcode ()
{
    ...
    $user_id = $_SESSION['user_id'];
    $action = $GLOBALS['action'];
	$url = 'http://'.$_SERVER['HTTP_HOST'].'/b2b2c/register.php';

    include_once (ROOT_PATH . 'includes/lib_transaction.php');

    $rs = $db->getOne("SELECT qrcode FROM " . $ecs->table('users') . " WHERE user_id = '$user_id'" );
	if(file_exists('themes/swtshop_buy/images/qrcode/'."$rs") && !empty($rs))
	{
        $qrcode = $rs;
	}
	else{
        $qrcode = qr_code($url,$user_id);
       
      $sql = "UPDATE" . $ecs->table('users') . "SET qrcode='$qrcode' where user_id=".$user_id;
      mysql_query($sql);
	}

	
    $smarty->assign('qrcode', $qrcode);
    $smarty->assign('myurl',$url.'?'.$user_id);
    $smarty->display('user_transaction.dwt');
}
//二维码生成函数
function qr_code($url,$user_id){
    $PNG_TEMP_DIR = dirname(__FILE__).DIRECTORY_SEPARATOR.'themes'.DIRECTORY_SEPARATOR.'swtshop_buy'.DIRECTORY_SEPARATOR.'images'.DIRECTORY_SEPARATOR.'qrcode'.DIRECTORY_SEPARATOR;
    //html PNG location prefix
    $PNG_WEB_DIR = 'themes/swtshop_buy/images/qrcode/';
    include "code/qrlib.php";

    if (!file_exists($PNG_TEMP_DIR))//判断文件夹是否存在
        mkdir($PNG_TEMP_DIR);
    ...
    if ($png)
    {
        ...
        // user data
        $filename = $PNG_TEMP_DIR.$user_id.md5($png.'|'.$errorCorrectionLevel.'|'.$matrixPointSize).'.png';
		$truename = $user_id.md5($png.'|'.$errorCorrectionLevel.'|'.$matrixPointSize).'.png';
        QRcode::png($png, $filename, $errorCorrectionLevel, $matrixPointSize, 2);
...
function action_tg_login_act ()
{
	...
	$_SESSION['takegoods_sn_swtshop'] = $tg_sn;
	$_SESSION['takegoods_id_swtshop'] = $tgrow['tg_id'];
...
/* 余额额支付密码_添加_START_www.swtshop.cn */
function action_check_surplus_open ()
{
	...
	$sql = 'SELECT `is_surplus_open`' . 'FROM `swt_users`' . 'WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'LIMIT 1';
...
function action_verify_surplus_password ()
{
	...
	$sql = 'SELECT COUNT( * )' . 'FROM `swt_users`' . 'WHERE `user_id` = \'' . $_SESSION['user_id'] . '\'' . 'AND `surplus_password` = \'' . md5($_GET['surplus_password']) . '\'';
	$count = $GLOBALS['db']->getOne($sql);
	echo $count;
	exit();
}
...
/* 账户安全_end_www.swtshop.cn */
/* 余额额支付密码_添加_END_www.swtshop.cn */
