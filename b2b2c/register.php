 ...
 * author:Minless
 * */
function action_check_introuser_exist ()
{
    $_LANG = $GLOBALS['_LANG'];
    $_CFG = $GLOBALS['_CFG'];
    $smarty = $GLOBALS['smarty'];
    ...
    $introUser = empty($_POST['introUser']) ? '' : $_POST['introUser'];
    $user = $GLOBALS['user'];

    if($user->check_user($introUser) || $user->check_email($introUser) || $user->check_mobile_phone($introUser)) {
        $where = [
            $user->check_user($introUser),
            $user->check_email($introUser),
            $user->check_mobile_phone($introUser)
        ];
        $rs = array_filter($where);
        echo $rs[0];
    }
    else{
        echo 'false';
...
function action_default ()
{
	...
	/*查看是否扫码或者分享链接的请求id
      auther:Minless Time:2018年3月16日11:22:07
    */
    $introuserid = empty($_REQUEST['u']) ? '' : $_REQUEST['u'];
    if(!empty($introuserid)) {
        $introusername = $db->getOne("SELECT user_name from".$ecs->table('users')."WHERE user_id =".$introuserid);
    }
    else{
        $introusername = '';
    }

    $smarty->assign('introusername',$introusername);
...
function action_register ()
{
	...
	else
	{
		...
		$other['introduce'] = isset($_POST['introduce']) ? $_POST['introduce'] : '';
