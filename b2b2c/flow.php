...
if ($_REQUEST['step'] == 'add_to_cart')
{
	...
	//www.swtshop.cn start add 2015-3-26
	$time_xg_now=gmtime();
	...
	//www.swtshop.cn end add 2015-3-26
	...
	$result['goods_id'] = $goods->goods_id;
	$sql_where = $_SESSION['user_id']>0 ? "user_id='". $_SESSION['user_id'] ."' " : "session_id = '" . SESS_ID . "' AND user_id=0 ";//添加 www.swtshop.cn
...
/* 代码增加_end  By www.swtshop.cn */
    ...
    else
    {
        ...
        elseif (!empty($_POST['act']) && $_POST['act'] == 'signup')
        {
            ...
            {
                /* 用户注册成功 */
				/* 四合一修改 start by www.swtshop.cn */
                ecs_header("Location: flow.php?step=checkout\n");
				/* 四合一修改 end by www.swtshop.cn */
                exit;
            }
            else
            {
...
elseif ($_REQUEST['step'] == 'checkout')
{
   ...
   // else if($_SESSION['extension_code'] == 'virtual_good')
    ...
    {
        /* 如果不完整则转向到收货人信息填写界面 */
		/* 代码修改_start  By  www.swtshop.cn  注释或删掉  */
        //ecs_header("Location: flow.php?step=consignee\n");   
        //exit;
...
elseif ($_REQUEST['step'] == 'change_surplus')
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* 代码修改 By  www.swtshop.cn #1305 Start */
//            if($total['amount_formated'] <= 0){
            if($total['amount'] <= 0)
            {
            /* 代码修改 By  www.swtshop.cn #1305 End */
    ...
    /* 代码修改 By  www.swtshop.cn #1854 Start */
    $result['surplus'] = round($result['surplus'] ,2);
    /* 代码修改 By  www.swtshop.cn #1854 End */
    $json = new JSON();
    die($json->encode($result));
}
...
{	
    ...
    foreach ($cart_goods_new as $ckey=>$cval){
	    ...
	    $order = array(
	        ...
	        /*增值税发票_删除_START_www.swtshop.cn*/
        	//'inv_type'        => $_POST['inv_type'],
        	//'inv_payee'       => trim($_POST['inv_payee']),
        	//'inv_content'     => $_POST['inv_content'],
			/*增值税发票_删除_END_www.swtshop.cn*/
		...
		$order['defaultbank'] = $_POST['www_swtshop_com_bank'] ? trim($_POST['www_swtshop_com_bank']) : "";
    ...
    //生成订单
	//$payment_www_com['www_ecshop68_com_alipay_bank'] = $_POST['www_swtshop_com_bank'] ? trim($_POST['www_swtshop_com_bank']) : "www_ecshop68_com";
	...
	//$GLOBALS['db'] -> query("unlock tables");
	    ...
	    if ($order['order_amount'] <= 0)
	    {
	        ...
	        foreach ($res AS $row)
	        {   
                    /* 代码增加_start  By www.swtshop.cn _sunlizhi*/
...
//                    }
                    ...
                    /* 代码增加_end  By www.swtshop.cn _sunlizhi*/
...
{
   /* 代码修改_start  By  www.swtshop.cn  将这块替换掉*/
...
{
    /* 查询：购物车中所有不可以单独销售的配件 */
	$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";//添加 www.swtshop.cn
