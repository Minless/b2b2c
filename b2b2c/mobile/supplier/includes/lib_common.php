 ...
 */
// 代码修改_start_derek20150129admin_goods  www.swtshop.cn
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.swtshop.cn
{
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.swtshop.cn
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.swtshop.cn
...
function Recordkeyword($word_www_swtshop_com, $items = 0, $searchengine = 'ecshop')
{
	if(strlen($word_www_swtshop_com) < 3 || strlen($word_www_swtshop_com) > 30 || strpos($word_www_swtshop_com, ' ') !== false) return;
	$sql_www_swtshop_com = "SELECT * FROM " .$GLOBALS['ecs']->table('keyword'). " WHERE searchengine='ecshop' AND word='$word_www_swtshop_com'";
	$r = $GLOBALS['db']->getRow($sql_www_swtshop_com);
	if($r)
	{
		$items = intval($items) ;   //www.swtshop.cn
        ...
        $sql_www_swtshop_com = "UPDATE " . $GLOBALS['ecs']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_www_swtshop_com);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_www_swtshop_com   = gb2py($word_www_swtshop_com);
		$sql_www_swtshop_com = "INSERT INTO " . $GLOBALS['ecs']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_www_swtshop_com', '$word_www_swtshop_com', '$letter_www_swtshop_com', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_www_swtshop_com);
		$w_id = $GLOBALS['db']->insert_id();
	}
	if (!empty($w_id))
	{
    	$ip_www_swtshop_com       = real_ip();
    	$area_www_swtshop_com     = ecs_geoip($ip);
    	$sql_www_swtshop_com = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_www_swtshop_com', '$area_www_swtshop_com')";
    	$GLOBALS['db']->query($sql_www_swtshop_com);
	}
}
/* 代码增加_END    by   www.swtshop.cn */
