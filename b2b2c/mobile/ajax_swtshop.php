...
if ($_REQUEST['act'] == 'tipemail')
{
	...
	if(!$word_www_swtshop_com ||  strlen($word_www_swtshop_com) > 30)
	{
	...
	$email_name_arr = explode("@", $word_www_swtshop_com);
    ...
    {
		if (stristr($email_domain, $word_www_swtshop_com))
		{
			$logdb[] = $email_domain;
		}
	}
	...
	if(count($logdb)==0)
	{
		$result_www_swtshop_com['content'] = '';
	}
	else
	{		
		$result_www_swtshop_com['content'] = $smarty->fetch('library/email_tip.lbi');
	}
