 ...
 * ECSHOP 购物流程函数库
 * ============================================================================
 * 版权所有 2005-2010 98ecshop商网通开发中心，并保留所有权利。
...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            " WHERE order_id = '$order_id'";  // 修改 by www.98ecshop.com 增加 package_attr_id 字段
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id']); // 修改 by www.98ecshop.com
        }
        $goods_list[] = $row;
    }
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            ...
            "AND rec_type = '$type'"; //这句SQL有修改 by www.98ecshop.com 增加一个 package_attr_id
    ...
    foreach ($arr as $key => $value)
    {
        ...
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id'], $value['package_attr_id']); //修改 by www.98ecshop.com
        }
    }
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改 by www.98ecshop.com 增加一个变量
        }
        $goods_list[] = $row;
    }
...
//    }
    ...
    $parent = array(
        ...
        'market_price'  => $package_pricea[0] ? $package_pricea[0] :  $package['market_package'], //修改 by www.98ecshop.com
        'goods_price'   => $package_pricea[1] ? $package_pricea[1] :  $package['package_price'], //修改 by www.98ecshop.com
		'package_attr_id' =>$package_attr_id, //增加 by www.98ecshop.com
        'goods_number'  => $num,
        'goods_attr'    => '',
        'goods_attr_id' => '',
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'";  //修改 by www.98ecshop.com 增加一条件
