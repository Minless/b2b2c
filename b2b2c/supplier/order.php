...
elseif ($_REQUEST['act'] == 'operate')
{
	...
	/*------------------------------------------------------ */
//-- start一键发货 增加 by www.swtshop.cn
/*------------------------------------------------------ */
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        if($_REQUEST['act'] == 'invoice_list' || (isset($_REQUEST['act_detail'])&&$_REQUEST['act_detail']=='invoice_query'))
        {
         ...
         /* 普通订单不显示虚拟团购订单 添加_START_www.swtshop.cn*/
        $where .= " AND o.extension_code != 'virtual_good'";
         /* 普通订单不显示虚拟团购订单 添加_END_www.swtshop.cn*/
		/*增值税发票_添加_END_www.swtshop.cn*/
        //综合状态
        switch($filter['composite_status'])
        {
