...
if ($_REQUEST['act'] == 'list_edit')
{
	...
	/*$sql="select * from swt_supplier_shop_config where code='jwdbj' and supplier_id=".$_SESSION['supplier_id'];
	$jwd=$db->getRow($sql);
	if(empty($jwd['id']))
	{ 
		$sql="insert into swt_supplier_shop_config(parent_id, code,type,sort_order,supplier_id) VALUES ('1','jwdbj','jwd','1','".$_SESSION['supplier_id']."')";
     }   */
...
elseif ($_REQUEST['act'] == 'post')
{
	...
	//$jd= substr($jd,7);
	//$wd= substr($wd,7);
	$sql="select * from swt_supplier_shop_config where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
	$jingdu=$db->getRow($sql);
	if(empty($jingdu['id']))
	{ 
		$sql="insert into swt_supplier_shop_config(parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','jingdu','jwd','".$jd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update swt_supplier_shop_config set value='".$jd."' where code='jingdu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }   
	
	$sql="select * from swt_supplier_shop_config where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
	$weidu=$db->getRow($sql);
	if(empty($weidu['id']))
	{ 
		$sql="insert into swt_supplier_shop_config(parent_id, code,type,value,sort_order,supplier_id) VALUES ('1','weidu','jwd','".$wd."','1','".$_SESSION['supplier_id']."')";
		@$db->query($sql);
	 }else{
		$sql="update swt_supplier_shop_config set value='".$wd."' where code='weidu' and supplier_id=".$_SESSION['supplier_id'];
		@$db->query($sql);
	 }
