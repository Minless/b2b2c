...
elseif ($_REQUEST['act'] == 'main')
{
    ...
    /*后台管理起始页_添加_START_www.swtshop.cn*/
...
//    $today['money'] = $db->GetOne('SELECT SUM(money_paid) AS money FROM ' .$ecs->table('order_info'). ' WHERE `add_time` BETWEEN '.$today_start.' AND '.$today_end.' AND supplier_id='.$_SESSION['supplier_id'].order_query_sql('finished'));
    ...
    $order['new_repay'] = $db->getOne('SELECT COUNT(*) FROM '.$ecs->table('back_order').' WHERE status_back < 6 AND status_back != 3 AND back_type = 4 AND supplier_id='.$_SESSION['supplier_id'].'');// 代码修改   By  www.swtshop.cn End
    //退货申请
