...
$tg_id       = $_SESSION['takegoods_id_swtshop'];
...
{
	$_SESSION['takegoods_id_swtshop']=0;
	$_SESSION['takegoods_sn_swtshop']='';
	header("Location:user.php?act=tg_login");
}
...
elseif ($_REQUEST['act'] == 'take')
{
	...
	$takegoods_sn_swtshop = $_SESSION['takegoods_sn_swtshop'];
	$takegoods_id_swtshop = isset($_POST['takegoods_id_swtshop'])  ? intval($_POST['takegoods_id_swtshop']) : 0;
    if (empty($takegoods_id_swtshop))
	{
		show_message('错误操作', '返回上一页', 'takegoods.php', 'error');
	}
	else
	{ 
		$sql = "select goods_ids from ". $ecs->table('takegoods_goods') ." where tg_id='$takegoods_id_swtshop' ";
		$goods_ids = $db->getOne($sql);
		if (empty($goods_ids))
		{
			$sql = "select tgtg.goods_ids from ". $ecs->table('takegoods') ." AS tg left join ". $ecs->table('takegoods_type_goods') .
				...
				" where tg.tg_id='$takegoods_id_swtshop' ";
			$goods_ids = $db->getOne($sql);
		}
		...
		$sql = "select tg.used_time, tt.type_money_count from ". $ecs->table('takegoods') ."AS tg left join ".$ecs->table('takegoods_type')."AS tt on tg.type_id = tt.type_id where tg.tg_id='$takegoods_id_swtshop' ";
	...
	$sql = "insert into ". $ecs->table('takegoods_order') . "(tg_id, tg_sn, user_id, goods_id, goods_name, country, province, city, district, address, consignee,mobile,email,add_time  )".
		" value('$takegoods_id_swtshop', '$takegoods_sn_swtshop', '$user_id', '$goods_id', '$goods_name', '$country', '$province','$city','$district', '$address', '$consignee' , '$mobile', '$email', '$add_time')";
	$db->query($sql);
	$tg_order_id = $db->insert_id();

	$add_time_sql = "select tg_order_id, used_time from ".$ecs->table('takegoods')." where tg_id='$takegoods_id_swtshop'";
	...
	$sql = "update ". $ecs->table('takegoods') ." set tg_order_id='$tg_order_id', used_time='$add_time'  where  tg_id='$takegoods_id_swtshop' ";
	$db->query($sql);
	...
	if ($_CFG['takegoods_send_email'] =='1' || $_CFG['takegoods_send_sms'] == '1' )
	{
		...
		{	
			...
			send_mail($_CFG['shop_name'], $admin_email, '卡号：'. $takegoods_sn_swtshop .' 提货成功', $content1, 0);
			send_mail($_CFG['shop_name'], $email, '卡号：'. $takegoods_sn_swtshop .' 提货成功', $content2, 0);
		}
