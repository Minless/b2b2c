...
<div id="filter_container" class="mfp-hide">
  ...
  <?php if ($this->_var['brand_list']): ?>
	...
	<?php $_from = $this->_var['brands']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'brand');$this->_foreach['brands_swtshop'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['brands_swtshop']['total'] > 0):
    foreach ($_from AS $this->_var['brand']):
        $this->_foreach['brands_swtshop']['iteration']++;
?>
...
?>
	...
	<?php $_from = $this->_var['filter_attr']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'attr');$this->_foreach['attr_list_swtshop'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['attr_list_swtshop']['total'] > 0):
    foreach ($_from AS $this->_var['attr']):
        $this->_foreach['attr_list_swtshop']['iteration']++;
?>
