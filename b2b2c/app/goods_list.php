...
if($act = 'get_goods')
{
	...
	if (!$smarty->is_cached($template, $cache_id))
	{
		...
		$sql = "SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ".
				...
				"GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC"; //此SQL语句增加字段 b.brand_logo,  By  www.swtshop.cn
		...
		$brand_have_logo = 0;  //代码增加   By    www.swtshop.cn
		...
		foreach ($brands AS $key => $val)
		{
			...
			$brands[$temp_key]['brand_id_swtshop'] = $val['brand_id'];
		...
		$smarty->assign('brand_zimu_swtshop', $brand_zimu);
		...
		if ($cat['filter_attr'] > 0)
		{
			...
			foreach ($filter_attr AS $fkey=>$fval)
			{
				if ($fval)
				{
					...
					$temp_arrt_url_swtshop =implode(".", $filter_attr_temp);
					if (strstr($fval, "-"))
					{
						$fval_array = explode("-", $fval);
						foreach ($fval_array AS $fval_key=> $fval_swtshop)
						{
							$cond_name .= ($fval_key ? "，" : "") . $filter_attr_name[$fkey][$fval_swtshop];
						}
					}
					else
					{
					...
					$condition[]=array(
							...
							'cond_name' =>$cond_name,
							'cond_url' =>build_uri('category', array('cid'=>$cat_id, 'bid'=>$brand, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$temp_arrt_url_swtshop, 'filter'=>$filter), $cat['cat_name'])
						);
				}
			}
			$attr_group_more_txt = "";
			$attr_group_more_id =0;
			foreach ($all_attr_list AS $k_swtshop=>$k_value)
			{
				if ($k_swtshop >1)
				{
...
function get_category_goods_count($children, $brand, $min, $max, $ext, $size, $page, $sort, $order,$is_promote,$filter,$lat,$log,$keywords,$sc_ds,$supplier_id,$extension_goods)
{
	...
	if(!empty($keywords))
	{
		...
		include_once(ROOT_PATH.'includes/lib_splitword_www_swtshop_com.php');
