...
$sql_where = $_SESSION['user_id']>0 ? "c.user_id='". $_SESSION['user_id'] ."' " : "c.session_id = '" . SESS_ID . "' AND c.user_id=0 ";
    $sql = "SELECT c.rec_id, c.user_id, c.goods_id, c.goods_name, c.goods_sn, c.goods_number, c.market_price, " .
			...
			" WHERE $sql_where " .
            " AND c.rec_type = '$type' $id_ext ";  //代码修改 By  www.swtshop.cn  增加一个 $id_ext , package_attr_id	
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0)
{
	...
	'add_time'   => gmtime(),   //代码增加   By  www.swtshop.cn
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND parent_id = 0 AND goods_attr = '" .get_goods_attr_info($spec). "' " .
                " AND extension_code <> 'package_buy' " .
				" AND user_id= '".$_SESSION['user_id']."'". //代码增加  By  www.swtshop.cn
 ...
 * @param   bool    $storage     减库存的时机，1，下订单时；0，发货时； 2，付款时
 */
...
function put_virtual_cart($order_id){
    /* 代码增加_虚拟团购_START  www.swtshop.cn */
