 ...
 * ECSHOP 公用函数库
 * ============================================================================
 * 版权所有 2010-2016 98ecshop商网通开发中心，并保留所有权利。
...
function cat_list($cat_id = 0, $selected = 0, $re_type = true, $level = 0, $is_show_all = true)
{
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order,c.cat_img, COUNT(s.cat_id) AS has_children ".
...
/* 代码增加 By  www.98ecshop.com Start */
/**
 * 过滤和排序所有分类，返回一个带有缩进级别的数组
 *
 * @access  private
...
function load_config()
{
    ...
    if ($data === false)
    {
        ...
        // 代码修改_start www.98ecshop.com
//         $arr['qq']                   = !empty($arr['qq']) ? $arr['qq'] : '';
//         $arr['ww']                   = !empty($arr['ww']) ? $arr['ww'] : '';
        $arr['qq'] = '';
        $arr['ww'] = '';
        $qq = $GLOBALS['db']->getAll("SELECT value FROM " . $GLOBALS['ecs']->table('shop_config') . " WHERE code = 'qq'");
        $ww = $GLOBALS['db']->getAll("SELECT value FROM " . $GLOBALS['ecs']->table('shop_config') . " WHERE code = 'ww'");
        foreach ($qq as $k => $v)
        {
            $arr['qq'] = $arr['qq'] . ',' . $v['value'];
        }
        foreach ($ww as $k => $v)
        {
            $arr['ww'] = $arr['ww'] . ',' . $v['value'];
        }
        // 代码修改_end www.98ecshop.com
        ...
        /* fulltext_search_add_START_www.98ecshop.com */
	$arr['fulltext_search']            = isset($arr['fulltext_search']) ? $arr['fulltext_search'] : 0;  
        /* fulltext_search_add_END_www.98ecshop.com */
 ...
 */
// 代码修改_start_derek20150129admin_goods  www.98ecshop.com
function get_brand_list($t = false)
// 代码修改_end_derek20150129admin_goods  www.98ecshop.com
{
...
{
/* 代码增加_虚拟团购_START  www.98ecshop.com */
    $add_date = gmtime();
...
/* 代码增加_虚拟团购_END  www.98ecshop.com */
...
/**
 *  清除手机指定后缀的模板缓存或编译文件
 *  wei2 增加 start by www.98ecshop.com
 * @access  public
 * @param  bool       $is_cache  是否清除缓存还是清出编译文件
 * @param  string     $ext       需要删除的文件名，不包含后缀
 *
...
/* wei2 增加 end by www.98ecshop.com */
/**
 * 清除模版编译文件
 *
 * @access  public
...
/* wei2 修改 end by www.98ecshop.com */
/**
 * 页面上调用的js文件
 *
 * @access  public
...
function get_package_goods($package_id, $package_attr_id='')
{
	...
	if ($package_attr_id)
	{
	 ...
	 // 下面SQL语句增加两个字段，注意逗号 , g.goods_thumb, g.shop_price   By  www.98ecshop.com
    ...
    while ($_row = $GLOBALS['db']->fetch_array($resource))
    {
		...
		/* 代码增加_start   By    www.98ecshop.com */
...
function Recordkeyword($word_www_98ecshop_com, $items = 0, $searchengine = 'ecshop')
{
	if(strlen($word_www_98ecshop_com) < 3 || strlen($word_www_98ecshop_com) > 30 || strpos($word_www_98ecshop_com, ' ') !== false) return;
	$sql_www_98ecshop_com = "SELECT * FROM " .$GLOBALS['ecs']->table('keyword'). " WHERE searchengine='ecshop' AND word='$word_www_98ecshop_com'";
	$r = $GLOBALS['db']->getRow($sql_www_98ecshop_com);
	if($r)
	{
		$items = intval($items) ;   //www.98ecshop.com
        ...
        $sql_www_98ecshop_com = "UPDATE " . $GLOBALS['ecs']->table('keyword') . " SET " .
                "items = '$items', " .
                "updatetime = '".gmtime()."', " .
				...
				"WHERE w_id = '".$r['w_id']."'";
		$GLOBALS['db']->query($sql_www_98ecshop_com);
		$w_id = $r['w_id'];
	}
	else
	{
		$letter_www_98ecshop_com   = gb2py($word_www_98ecshop_com);
		$sql_www_98ecshop_com = "INSERT INTO " . $GLOBALS['ecs']->table('keyword') . " (searchengine, word, keyword, letter, items, updatetime, total_search, ".
		"month_search, week_search, today_search, status) " .
                " VALUES ('$searchengine', '$word_www_98ecshop_com', '$word_www_98ecshop_com', '$letter_www_98ecshop_com', '$items', '".gmtime()."', '1', '1', '1', '1', '1')";
        $GLOBALS['db']->query($sql_www_98ecshop_com);
		$w_id = $GLOBALS['db']->insert_id();
	}
	if (!empty($w_id))
	{
    	$ip_www_98ecshop_com       = real_ip();
    	$area_www_98ecshop_com     = ecs_geoip($ip);
    	$sql_www_98ecshop_com = 'INSERT INTO ' . $GLOBALS['ecs']->table('keyword_area') . ' ( ' .
                'w_id, access_time, ip_address, area) VALUES (' .
                "'$w_id', '".gmtime()."', '$ip_www_98ecshop_com', '$area_www_98ecshop_com')";
    	$GLOBALS['db']->query($sql_www_98ecshop_com);
	}
}
/* 代码增加_END    by   www.98ecshop.com */
