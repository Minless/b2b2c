...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('article_cat')."(cat_name, cat_type, cat_desc,keywords, parent_id, sort_order, show_in_nav, path_name)
           VALUES ('$_POST[cat_name]', '$cat_type',  '$_POST[cat_desc]','$_POST[keywords]', '$_POST[parent_id]', '$_POST[sort_order]', '$_POST[show_in_nav]', '$_POST[path_name]')";//代码修改 By  www.98ecshop.com 增加新字段 path_name 和  $_POST[path_name]
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT cat_id, cat_name, cat_type, cat_desc, show_in_nav, keywords, parent_id,sort_order, path_name FROM ".
           $ecs->table('article_cat'). " WHERE cat_id='$_REQUEST[id]'";//代码修改 By www.98ecshop.com 增加一个path_name 字段
    $cat = $db->GetRow($sql);
