...
require_once(ROOT_PATH . '/' . ADMIN_PATH. "/nusoap/nusoap.php");   //代码增加  By  www.98ecshop.com
...
{
     /* 修改 by www.98ecshop.com 百度编辑器 begin */
    //include_once(ROOT_PATH . 'includes/fckeditor/fckeditor.php'); // 包含 html editor 类文件
    /* 修改 by www.98ecshop.com 百度编辑器 end */

	// 代码增加_start_derek20150129admin_goods  www.98ecshop.com

	include_once(ROOT_PATH . '/includes/Pinyin.php');

	// 代码增加_end_derek20150129admin_goods  www.98ecshop.com
    ...
    // 获取仓库列表
    $sql = "SELECT * FROM " . $ecs->table('ware') . " ORDER BY sort_order ASC";
    $ware_list = $db->getAll($sql);

    /* 拆分商品名称样式 */
    $goods_name_style = explode('+', empty($goods['goods_name_style']) ? '+' : $goods['goods_name_style']);

    /* 创建 html editor */
   create_html_editor('goods_desc', htmlspecialchars($goods['goods_desc'])); /* 修改 by www.98ecshop.com 百度编辑器 */
	
   //是否多选
   $isarr=explode(',', $goods['warehouse']);
   $i=0;
   foreach ($ware_list as $arr){
   
   	if (in_array($arr[id], $isarr)) {
   		$ware_list[$i]['isshow']=1;
   	}else{
   		
   		$ware_list[$i]['isshow']=0;
   	}
   	$i++;
   }
	...
	$smarty->assign('brand_list', get_brand_list());
	$smarty->assign('ware_list', $ware_list);
	// 代码增加_start_derek20150129admin_goods  www.98ecshop.com
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
  ...
  $warehouse= implode(',', $_POST['warehouse']);
    ...
    if ($is_insert)
    {
        if ($code == '')
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, rank_integral, suppliers_id, cost_price,warehouse)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    ...
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$rank_integral', '$suppliers_id', '$cost_price','$warehouse')";
        }
        else
        {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral, cost_price,warehouse)" .
                "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote', '$zhekou', '$promote_price', ".
                    "'$promote_start_date', '$promote_end_date', '$is_buy','$buymax','$buymax_start_date','$buymax_end_date', '$goods_img', '$goods_thumb', '$original_img', ".
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number',".
                    " '$warn_number', '$_POST[integral]', '$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, ".
                    " '$_POST[goods_desc]', '" . gmtime() . "', '". gmtime() ."', '$goods_type', '$code', '$rank_integral', '$cost_price','$warehouse')";
        }
    }
    else
    {
