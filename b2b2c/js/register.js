...
function checkUsers(introUser,callback) {

	var introuserObj = null;
	if(typeof(introUser)=='object'){
		introuserObj = $(introUser);

        introUser = introuserObj.val();
        //console.log(introUser);
	}
	if(introUser ==''){
        document.getElementById('conform_introduce_notice').innerHTML = '您暂且没有推荐人';
        document.getElementById('conform_introduce_notice').style.color = '#093';

	}else{
        checkIntroUserExist(introUser, callback);
	}
}
/*
* 模仿原生ecshop代码添加
* author:Minless
* Time:2018-03-14 14:15:01
* */
function checkIntroUserExist(introUser,callback) {

   /* console.log(introUser);
    console.log('下面开始ajax');*/

    $.post('register.php?act=check_introuser_exist', {
        introUser: introUser
    }, function(result) {
        if(result=='false')
		{
            console.log('用户不存在');
            document.getElementById('conform_introduce_notice').innerHTML = '推荐人不存在';
            document.getElementById('conform_introduce_notice').style.color = '#E31939';
		}
		else{
            console.log('推荐人存在');
            document.getElementById('conform_introduce_notice').innerHTML = '推荐人可用';
            document.getElementById('conform_introduce_notice').style.color = '#093';
