...
if (!$smarty->is_cached('index.dwt'))
{
	...
	$daygoods = get_recommend_goods('hot');
	
	$smarty->assign('daygoods',       $daygoods[0]); //天天抢
	...
	$smarty->assign('comments',        get_comments());             //晒单
...
function get_comments()
{
   ...
   $sql = 'SELECT a.*,b.* FROM '. $GLOBALS['ecs']->table('comment') .
    ...
    ' AS a,'. $GLOBALS['ecs']->table('goods') .'AS b WHERE a.status = 1 AND a.comment_rank = 5 AND a.id_value=b.goods_id '.

    ' ORDER BY a.add_time DESC';

    $sqls = 'SELECT u.headimg FROM '. $GLOBALS['ecs']->table('comment') .' 
    AS a,'. $GLOBALS['ecs']->table('users') .'AS u WHERE a.status = 1 AND a.comment_rank = 5 AND a.user_id = u.user_id ';

    $result = $GLOBALS['db']->getAll($sqls);
    // var_dump($row);die;

    $pravis = array();
    foreach ($result as $key => $value) {

    	$pravis[$key]['headimg']  = $value['headimg'];
    }
  ...
  foreach ($res AS $idx => $row)
  {
   ...
   $comments[$idx]['goods_id']		= $row['goods_id'];
   ...
   $comments[$idx]['headimg']       = $pravis[$idx]['headimg'];
...
function index_get_invoice_query()
{
    ...
    ' .' LEFT JOIN ' . $GLOBALS['ecs']->table('shipping') . ' AS s ON s.shipping_id = o.shipping_id' ." WHERE invoice_no > '' AND shipping_status = " . SS_SHIPPED .' ORDER BY shipping_time DESC LIMIT 10';
...
{
    $sql = 'SELECT a.article_id, a.title, ac.cat_name, a.add_time, a.file_url, a.open_type, ac.cat_id, ac.cat_name ' . ' FROM 
    ' . $GLOBALS['ecs']->table('article') . ' AS a, 
    ' .$GLOBALS['ecs']->table('article_cat') . ' AS ac
    ' .' WHERE a.is_open = 1 AND a.cat_id = ac.cat_id AND ac.cat_type = 1' .' ORDER BY a.article_type DESC, a.add_time DESC LIMIT ' . $GLOBALS['_CFG']['article_number'];
    $res = $GLOBALS['db']->getAll($sql);
